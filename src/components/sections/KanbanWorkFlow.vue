...
<div class="w-full h-full flex overflow-auto">
    <div class="w-1/3 border-r flex-shrink-0 text-xs overflow-auto" v-for="column in columns" :key="column.slug" @dragover.prevent @drop="onDragAndDropEnter(column)">
        ...
        <div class="m-2 p-5 bg-white rounded-lg h-auto border shadow" v-for="card in cards.filter((el) => el.status === column.slug)" :key="card.id" :id="card.id" draggable="true" @dragstart="onDragStart(card.id)">
...
async function onDragAndDropEnter(newStatusSlug) {
    const cardIndex = cards.value.findIndex(card => card.id === draggedCardID.value)
