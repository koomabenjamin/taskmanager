<template>
  ...
  <div class="w-full h-full flex overflow-auto">
    <div
      ...
      :key="column"
    >
      <!-- <Card draggable="true" @dragstart="hide(`card-${column}`)" :id="`card-${column}`" v-for="card in deletedTaskCards.filter((el) => el.status === column)" /> -->
      ...
      <div
        ...
        :id="card.id"
        v-for="card in deletedTaskCards.filter((el) => el.status === column.slug)"
        :key="card"
      >
