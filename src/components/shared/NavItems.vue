...
<div class="space-y-4">
  ...
  <div>
    ...
    ></CustomSelect>
      ...
      <form @submit.prevent="submitMemberForm" class="grid grid-cols-1 gap-4" v-if="selectedForm === 'member'">
  <div>
    <label for="name" class="block text-sm font-medium text-gray-900">Name:</label>
    <input id="name" name="name" type="text" v-model="name" placeholder="Name" required class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-lime-500 focus:border-lime-500 sm:text-sm" />
  </div>

  <div>
    <label for="email" class="block text-sm font-medium text-gray-900">Email Address:</label>
    <input id="email" name="email" type="email" v-model="email" placeholder="E-mail Address" required class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-lime-500 focus:border-lime-500 sm:text-sm" />
  </div>

  <div class="col-span-1">
    <button type="submit" class="w-full flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-lime-500 hover:bg-lime-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-lime-500">
      <PlusIcon class="h-5 w-5 text-white mr-2" aria-hidden="true" /> Add New Member
    </button>
  </div>
</form>

      <!-- <form @submit.prevent="submitMemberForm" class="grid grid-cols-2 gap-2 -space-y-0" v-if="selectedForm === 'member'">
  ...
  <div>
    ...
    <Input id="name" name="name" type="text" v-model="name" placeholder="Name" required />
  </div>
                    ...
                    <label for="email" class="block text-sm font-medium text-gray-900">Email Address:</label>
                    <Input id="email" name="email" type="email" v-model="email" placeholder="E-mail Address" required />
                
        <Button type="submit" label="Add New Member" icon="PlusIcon" color="bg-lime-500 text-white col-span-2" size="xl" />
...
import { fetchAllMembersData,submitMemberData, allMembers } from '@/services/memberService';
